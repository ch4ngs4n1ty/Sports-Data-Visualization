function NflField({data}) {
  ...
  return <section className="nf-field"><div className="nf-field-heading"><b>{team?.abbr || 'Field'} {f?.text || 'Position unavailable'}</b><span>Offense moves →</span></div>
    <svg viewBox="0 0 600 270" role="img" aria-label={`Football field. Last reported position: ${f?.text || 'unavailable'}. Offense moves left to right.`}>
      <rect width="600" height="270" rx="9" fill="#103c2c"/><rect x="0" width="50" height="270" fill="#122b25"/><rect x="550" width="50" height="270" fill="#122b25"/>
      ...
      <text transform="translate(29 135) rotate(-90)" textAnchor="middle" fill="#9ab8a9" fontSize="14">OWN END ZONE</text><text transform="translate(579 135) rotate(90)" textAnchor="middle" fill="#9ab8a9" fontSize="14">OPPONENT</text>
    ...
    </svg><small>Blue: line of scrimmage · Gold: first down / goal · Last reported play position</small>
  </section>;
}
function NflLiveTab({gameInfo}) {
  ...
  React.useEffect(()=>{let dead=false,timer,controller,busy=false;
    async function load(){if(dead || document.hidden || busy)return;busy=true;controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),12000);
      try {const r=await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/nfl/summary?event=${encodeURIComponent(gameInfo.eventId)}`,{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error();const d=NflLive.normalize(await r.json());if(!dead){setData(d);setError('');}}
      catch(e){if(!dead)setError('Feed unavailable. Showing last received data; percentages are paused.');}
      finally{clearTimeout(timeout);busy=false;if(!dead && auto)timer=setTimeout(load,15000);}}
    const visible=()=>{clearTimeout(timer);if(!document.hidden)load();};document.addEventListener('visibilitychange',visible);load();return()=>{dead=true;clearTimeout(timer);controller?.abort();document.removeEventListener('visibilitychange',visible);};
  ...
  const card=(p,following=false)=>{const r=NflLive.probability(data,p,dist),k=NflLive.key(p),saved=follows.some(x=>NflLive.key(x)===k),h=history[k]||[];return <div className="nf-pick" key={k}><div><b>{p.label}</b><strong className={stale?'nf-muted':''}>{percent(r)}</strong></div><meter min="0" max="1" value={r?.win || 0} aria-label={`${p.label} estimated chance`}/><small>{r?.settled?(p.market==='PLAYER'?'Final result':'Regulation result'):stale?'Paused / stale estimate':p.market==='PLAYER'?'Rough pace estimate':'Estimated chance'}{r?.push>=.0005?` · ${(r.push*100).toFixed(1)}% ${p.market==='WIN'?'tie':'push'}`:''}{r?.value!=null?` · ${r.value} yards now`:''}</small>{following && h.length>1 && <small>{((h.at(-1)-h[0])*100).toFixed(1)} percentage points since following this visit</small>}<button disabled={!following && (!r || stale || (!saved && follows.length>=12))} onClick={()=>toggle(p)}>{saved?'✓ Following · remove':'+ Follow'}</button></div>;};
  return <div className="nf-live"><header className="nf-score"><div><img src={data.away.logo} alt=""/><b>{data.away.abbr}</b><strong>{data.away.score??'—'}</strong></div><p>{data.status}<small>{stale?'● PAUSED / STALE':data.final?'FINAL':'● FREE ESPN FEED'}</small></p><div><strong>{data.home.score??'—'}</strong><b>{data.home.abbr}</b><img src={data.home.logo} alt=""/></div></header>
    <div className="nf-toolbar"><span>Checked {Math.floor(age/1000)}s ago · polls every 15s · provider delay unknown</span><button onClick={()=>setAuto(v=>!v)}>{auto?'Pause':'Resume'}</button><button onClick={()=>setRefresh(n=>n+1)}>Refresh</button></div>{error && <p role="status">{error}</p>}
